...
public class CompetitionJdbcDao extends SpringJdbcDao<Competition> implements CompetitionDao {

	public CompetitionJdbcDao(final DataSource dataSource,  final DataFieldMaxValueIncrementer incrementer)
	{
	...
	public Competition getCompetition(long id) {
		...
		try {
			return jdbcTemplate.queryForObject(SQL_GET_COMPETITION, namedParameters, new CompetitionRowMapper());
	...
	private class CompetitionRowMapper implements RowMapper<Competition>
	{
		...
		public Competition mapRow(ResultSet rs, int arg1) throws SQLException {
			...
			return new Competition(
					rs.getLong("ID"),
					rs.getString("NAME"),
					EnumUtils.getEnumFromString(CompetitionType.class, rs.getString("COMPETITION_TYPE"))
					);
		}
	}
